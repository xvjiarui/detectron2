...
def point_sample_fine_grained_ensemble_features(
    features_list, feature_scales, boxes, point_coords, *, offset=(0.0, 0.0), mode="bilinear"
):
    """
    Get features from feature maps in `features_list` that correspond to specific point coordinates
        inside each bounding box from `boxes`.
    ...
    Args:
        ...
        point_coords (Tensor): A tensor of shape (R, P, 2) that contains
            [0, 1] x [0, 1] box-normalized coordinates of the P sampled points.
        offset (tuple[float]): Coordinate offset.
    ...
    point_features = []
    offset = _as_tensor(offset).to(point_coords.device)
    for idx_img, point_coords_wrt_image_per_image in enumerate(split_point_coords_wrt_image):
        point_features_per_image = []
        for idx_feature, feature_map in enumerate(features_list):
            h, w = feature_map.shape[-2:]
            scale = _as_tensor([w, h]) / feature_scales[idx_feature]
            point_coords_scaled = (point_coords_wrt_image_per_image + offset) / scale.to(
                feature_map.device
            )
            point_features_per_image.append(
                point_sample(
