    ...
    def _forward_mask_point(self, features, mask_coarse_logits, instances):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                mask_grids = generate_regular_grid_point_coords(
                    mask_coarse_logits.size(0),
                    self.mask_coarse_side_size,
                    mask_coarse_logits.device,
                )
                mask_grids = mask_grids.transpose(1, 2).reshape(
                    mask_grids.size(0), 2, self.mask_coarse_side_size, self.mask_coarse_side_size
                )
                if self.cell_decode:
                    mask_coarse_features_list = [features[k] for k in self.mask_coarse_in_features]
                    cell_code = point_sample_cell(
                        mask_coarse_features_list, features_scales, proposal_boxes, point_coords
                    ).transpose(1, 2)
            ...
            if self.local_ensemble:
                ...
                # swap 0<->3, 1<->2
                ensemble_weights.reverse()
                point_logits *= ensemble_weights[0]
                for i in range(1, len(ensemble_weights)):
                    point_logits += ensemble_weights[i] * ensemble_point_logits[i]

                point_logits /= sum(ensemble_weights) + 1e-9
            ...
            mask_logits = mask_coarse_logits.clone()
            mask_grids = generate_regular_grid_point_coords(
                mask_logits.size(0), mask_logits.size(2), mask_logits.device
            )
            mask_grids = mask_grids.transpose(1, 2).reshape(
                mask_grids.size(0), 2, *mask_logits.shape[2:]
            )
            for subdivions_step in range(self.mask_point_subdivision_steps):
                mask_logits = interpolate(
                    mask_logits, scale_factor=2, mode="bilinear", align_corners=False
                ...
                if self.cell_decode:
                    mask_coarse_features_list = [features[k] for k in self.mask_coarse_in_features]
                    cell_code = point_sample_cell(
                        mask_coarse_features_list, features_scales, pred_boxes, point_coords
                    ).transpose(1, 2)
                    assert cell_code.size(1) == 2
