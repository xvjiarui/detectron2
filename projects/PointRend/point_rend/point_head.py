...
class LIIFPointHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        The following attributes are parsed from config:
            ...
            coarse_pred_each_layer: if True, coarse prediction features are concatenated to each
                layer's input
        """
        super(LIIFPointHead, self).__init__()
        # fmt: off
        num_classes                 = cfg.MODEL.POINT_HEAD.NUM_CLASSES
        fc_dim                      = cfg.MODEL.POINT_HEAD.FC_DIM
        num_fc                      = cfg.MODEL.POINT_HEAD.NUM_FC
        cls_agnostic_mask           = cfg.MODEL.POINT_HEAD.CLS_AGNOSTIC_MASK
        location_dim                = 4 if cfg.MODEL.POINT_HEAD.CELL_DECODE else 2
        self.coarse_pred_each_layer = cfg.MODEL.POINT_HEAD.COARSE_PRED_EACH_LAYER
        self.location_each_layer    = cfg.MODEL.POINT_HEAD.LOCATION_EACH_LAYER
        input_channels              = input_shape.channels
        # fmt: on

        fc_dim_in = input_channels + num_classes + location_dim
        self.fc_layers = []
        for k in range(num_fc):
            fc = nn.Conv1d(fc_dim_in, fc_dim, kernel_size=1, stride=1, padding=0, bias=True)
            self.add_module("fc{}".format(k + 1), fc)
            ...
            fc_dim_in = fc_dim
            fc_dim_in += num_classes if self.coarse_pred_each_layer else 0
            fc_dim_in += location_dim if self.location_each_layer else 0
    ...
    def forward(self, fine_grained_features, coarse_features, location_code):
        x = torch.cat((fine_grained_features, coarse_features, location_code), dim=1)
        for layer in self.fc_layers:
            x = F.relu(layer(x))
            if self.coarse_pred_each_layer:
                x = cat((x, coarse_features), dim=1)
            if self.location_each_layer:
                x = cat((x, location_code), dim=1)
        return self.predictor(x)
